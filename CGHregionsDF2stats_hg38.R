#this is the value of filenameOfCGHregionsOutput, whole path to the file
#filenameOfCGHregionsOutput <- "/DATA/share/pipelines/lgleon/organoids_rawdata/DNA/old_organoids/fixingCGHregionsDF2stats/stats/30kb.point01percentsmoothing.CGHregions.tab"
#cghregiosn if the data frame itself
#cghregions_df <- read.delim(filenameOfCGHregionsOutput)
#binSize <- 30   # at the genninng of the code
...
chromosome_lengths_hg38 <- read.csv("/home/l.gonzalez/Notebooks/chr_hg38_len-size/hg38.chrom.sizes", header=FALSE, sep = '\t')
centromere_locations_hg38 <- read.csv("/home/l.gonzalez/Notebooks/chr_hg38_len-size/centromere_locations_hg38.csv", sep = ';')
#hg38  
#chromosome_lengths_hg38 <- read.table("/home/l.gonzalez/Notebooks/chr_hg38_len-size/hg38.chrom.sizes", header=FALSE, sep = '\t')
#centromere_locations_hg38 <- read.table("/home/l.gonzalez/Notebooks/chr_hg38_len-size/cytoBand.txt", sep = '\t')
...
## data from hg38 is downloaded but this part is not ready yet, use the locals files in the sever
...
#writeBin(x, destfile, useBytes = TRUE)
#centromere_locations_hg38 <- read.csv(destfile)
...
for (i in 1:rownum) {
  ...
  if(i == rownum || cghregions_df[i+1,1]-1 == cghregions_df[i,1]) # this is last pos or next pos is next chromosome
    cghregions_df[i,"dist"]= chromosome_lengths_hg38[chromosome_lengths_hg38[,1]==paste("chr",cghregions_df[i,1],sep=""),2] - cghregions_df [i,3] - binSize
  else
...
for (j in 1:dim(resultDF)[1]) {
  if (resultDF[j,"end"] <= centromere_locations_hg38[centromere_locations_hg38[,"chromosome"] == paste("chr",resultDF[j,"chr"],sep=""),"centromere.position"])
    resultDF[j,"chrarm"] = "p"
  else if (resultDF[j,"start"] > centromere_locations_hg38[centromere_locations_hg38[,"chromosome"] == paste("chr",resultDF[j,"chr"],sep=""),"centromere.position"])
    resultDF[j,"chrarm"] = "q"
  else { 
    arm_annotation = "c"
    if (resultDF[j,"start"] <= centromere_locations_hg38[centromere_locations_hg38[,"chromosome"] == paste("chr",resultDF[j,"chr"],sep=""),"centromere.position"])
...
for (j in 1:dim(resultDF)[1]) { # for every row
  ...
  if(chrarm == 'pcq') {
    centro = centromere_locations_hg38[centromere_locations_hg38$chromosome == paste("chr",chr, sep=""),"centromere.position"]
